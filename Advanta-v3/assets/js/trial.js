...
async function initializeTrials() {
  try {
    ...
    // Render dashboard trial progress
      ...
      return `
            <div class="inventory-item">
                <div class="item-meta">
                    ...
                    <div class="item-subtext">${escapeHtml(trial.cropName || trial.cropType || "-")} · ${escapeHtml(trial.trialType || "-")} · ${startDate} to ${endDate}</div>
                    <div class="item-subtext">${areaCount} area(s) · ${trial.parameters ? trial.parameters.length : 0} parameter(s)</div>
                </div>
                <div class="item-actions">
                    <button class="edit-btn" data-id="${trial.id}" title="Edit">
                        <span class="material-symbols-rounded">edit</span>
                    </button>
                    <button class="delete-btn" data-id="${trial.id}" title="Delete">
...
function openEditTrialModal(trialId) {
  ...
  populateTrialLocations();
  document.getElementById("trialLocation").value = trial.locationId || "";
...
function validateLocationSection() {
  const locationId = document.getElementById("trialLocation").value;
      ...
      return `
            <div style="background: var(--bg-tertiary); padding: 0.75rem; border-radius: 6px; margin-bottom: 0.75rem;">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem;">
                    <strong style="font-size: 0.95rem;">${escapeHtml(area.name)}</strong>
                    <button type="button" onclick="removeArea(${index})" style="background: var(--danger); color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 6px; cursor: pointer; font-size: 0.875rem;">
                        Remove
                    </button>
                </div>
...
async function saveTrial() {
  ...
  try {
    ...
    const locationCoords = selectedOption.dataset.coords;
...
function selectLine(areaIndex, paramId, lineId, repIndex) {
  ...
  if (runTrialState.currentAreaIndex !== null && runTrialState.currentParamId && runTrialState.currentLineId) {
    ...
    // Update nav tree and progress immediately after saving
    renderRunTrialNavTree();
    updateRunTrialProgress();
  }
  ...
  // Update nav active state using unique key
  document.querySelectorAll(".run-nav-line").forEach((el) => el.classList.remove("active"));
  const uniqueKey = `${areaIndex}_${paramId}_${lineId}_${repIndex}`;
  const activeEl = document.querySelector(`.run-nav-line[data-unique-key="${uniqueKey}"]`);
        ...
        rep.forEach((row) => {
          row.forEach((cell) => {
            ...
            if (!response?.photos?.length) {
              missingPhotos.push(`${area.name || `Area ${areaIndex + 1}`} - ${param.name} - ${cell.name} (Rep ${repIndex + 1})`);
            }
          });
        });
      });
